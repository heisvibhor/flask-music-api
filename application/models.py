...
class User(db.Model):
    ...
    user_type: Mapped[str] = mapped_column(db.CheckConstraint('user_type in ("USER", "CREATOR", "ADMIN")'),
                                           default="USER")
    playlists: Mapped[list["Playlist"]] = relationship(back_populates='user', cascade="save-update")
    language: Mapped[str] = mapped_column(db.ForeignKey("language.name"), nullable=False)
...
class Creator(db.Model):
    ...
    created_at: Mapped[datetime] = mapped_column(server_default=db.sql.func.now())
    disabled: Mapped[bool] = mapped_column(default=False)
    policy_violate: Mapped[str] = mapped_column()
    image: Mapped[str] = mapped_column()
    songs: Mapped[list["Song"]] = relationship(back_populates='creator', cascade="save-update")
    albums: Mapped[list["Album"]] = relationship(back_populates='creator', cascade="save-update")

class SongLikes(db.Model):

    song_id: Mapped[int] = mapped_column(db.ForeignKey("song.id"), primary_key=True, nullable=False)
...
class Song(db.Model):
    ...
    creator_id: Mapped[int] = mapped_column(db.ForeignKey("creator.id"), nullable=False)
    lyrics: Mapped[str] = mapped_column(db.Text)
    created_at: Mapped[datetime] = mapped_column(server_default=db.sql.func.now())
    audio: Mapped[str] = mapped_column()
    image: Mapped[str] = mapped_column()
    views: Mapped[int] = mapped_column(default=0)
    genre: Mapped[str] = mapped_column(db.ForeignKey("genre.name"), nullable=False)
    language: Mapped[str] = mapped_column(db.ForeignKey("language.name"), nullable=False)
    ...
    creator: Mapped["Creator"] = relationship(back_populates='songs', cascade="save-update")
...
class Playlist(db.Model):
    ...
    user: Mapped["User"] = relationship(back_populates='playlists', cascade="save-update")
...
class Album(db.Model):
    ...
    creator: Mapped["Creator"] = relationship(back_populates='albums', cascade="save-update")


class SongPlaylist(db.Model):
    song_id: Mapped[int] = mapped_column(db.ForeignKey("song.id"), primary_key=True, nullable=False)
    playlist_id: Mapped[int] = mapped_column(db.ForeignKey("playlist.id"), primary_key=True, nullable=False)

class AlbumSong(db.Model):
    song_id: Mapped[int] = mapped_column(db.ForeignKey("song.id"), primary_key=True, nullable=False)
    album_id: Mapped[int] = mapped_column(db.ForeignKey("album.id"), primary_key=True, nullable=False)
